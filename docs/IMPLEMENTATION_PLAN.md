...
1. PostgreSQL Compose, Flyway 테이블·로컬 시드와 JPA 모델을 구현했다. 사용자 요청에 따라 실행 검증은 보류했다.
2. DB 실행·마이그레이션·JPA 매핑과 데이터 유지 여부를 사용자가 확인했다.
3. 문의 목록·상세·담당자 API와 검색·필터·정렬·페이지 처리를 구현했다. 사용자 검증 대기 중이다.
4. Vue의 실제 API 모드와 개발 프록시를 연결했다. 사용자 검증 대기 중이다.
5. 상태·담당자 변경, 메모 저장과 변경 이력을 구현했다. 사용자 검증 대기 중이다.
6. 문의 원본에서 대시보드 집계를 계산해 Vue 대시보드와 연결한다.
7. 조회·변경 권한 정책과 프론트엔드 영향 범위를 합의한 뒤 인증을 구현한다.
8. 동시 수정·중복 요청·DB 실패를 재현하고 정책과 해결책을 비교해 검증한다.
