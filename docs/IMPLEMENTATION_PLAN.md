...
2. DB 실행·마이그레이션·JPA 매핑과 데이터 유지 여부를 확인한다.
3. 문의 목록·상세 API와 검색·필터·정렬·페이지 처리를 구현한다.
4. Vue의 실제 API 모드와 개발 프록시를 연결한다. 담당자 조회 등 상세 화면의 추가 요청도 확인한다.
5. 상태·담당자 변경, 메모 저장, 변경 이력과 대시보드 집계를 연결한다.
6. 조회·변경 권한 정책과 프론트엔드 영향 범위를 합의한 뒤 인증을 구현한다.
7. 동시 수정·중복 요청·DB 실패를 재현하고 정책과 해결책을 비교해 검증한다.
